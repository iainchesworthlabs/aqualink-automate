...
| Home Assistant add-on wrapper (repo-root `repository.yaml` + `aqualink-automate/`; dev harness + art under `homeassistant/`) — options/schema, `run.sh` flag mapping, base-image tag | `docs/homeassistant-addon.md` **and** `aqualink-automate/DOCS.md`; add/rename a `config.yaml` option → also update **every** `translations/<lang>.yaml` (configuration:/network: keys); **only edit the stable `aqualink-automate/` folder — `aqualink-automate-edge/` is GENERATED** by `scripts/gen-homeassistant-edge-addon.ps1` (re-run + commit after any change); keep each channel's `config.yaml` `version` == the app release version — the Dockerfile derives its base-image tag from it via `BUILD_VERSION`, and there is **no** `build.yaml` (CI `Home Assistant Add-on` checks the key is readable + the edge channel has no drift; `release.yml` is what pins the released channel to the release version) |
