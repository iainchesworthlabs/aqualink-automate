...
// Regression: the OPTIONS payload is a bit-mask and every flag must decode from its own
// bit. The decode used to static_cast the whole byte to the flags struct, which is
// parenthesised aggregate initialisation: the byte was converted to bool into the FIRST
// member and the other seven were value-initialised to false. So HasCleaner read true for
// any non-zero byte and Spillover / TwoSpeedPump / the rest could never be true at all --
// which silently disabled the spillover handling downstream. 0x01 and 0x00 (the two cases
// covered above) are the only bytes that look correct under the broken decode.
BOOST_AUTO_TEST_CASE(TestDeserialize_Options_SpilloverBitOnly_DoesNotReadAsCleaner)
{
	SerialAdapterMessage_DevStatus msg;
	// Byte 0x04 = bit 2 only -> HasSpillover, and specifically NOT HasCleaner.
...
BOOST_AUTO_TEST_CASE(TestDeserialize_Options_EachBitMapsToItsOwnFlag)
{
	// Every bit set: all eight flags must be true.
	{
		SerialAdapterMessage_DevStatus msg;
	...
	// Each bit in isolation lands on exactly the flag declared at that position.
	{
		SerialAdapterMessage_DevStatus msg;
		auto pkt = MakePacket(0x48, 0x41, { 0x01, 0x00, 0x80, 0x00 });
		BOOST_CHECK(msg.DeserializeContents(std::span<const uint8_t>(pkt)));
		BOOST_REQUIRE(msg.Options().has_value());
		auto opts = msg.Options().value();
		BOOST_CHECK(opts.ExtraDelayForHeatPump);
	...
	{
		SerialAdapterMessage_DevStatus msg;
		auto pkt = MakePacket(0x48, 0x41, { 0x01, 0x00, 0x40, 0x00 });
		BOOST_CHECK(msg.DeserializeContents(std::span<const uint8_t>(pkt)));
		BOOST_REQUIRE(msg.Options().has_value());
		auto opts = msg.Options().value();
		BOOST_CHECK(opts.CommonHeaterForSpaAndPool);
	...
	{
		// A cleaner-and-spillover panel: both bits, nothing else.
		SerialAdapterMessage_DevStatus msg;
